...
def starType(S,node,sizeOfSample): 
    ...
    right = retrive(S, go_right(node,sizeOfSample).location)
    left = retrive(S, go_left(node,sizeOfSample).location)
    node_list = [node,
                 right,
                 retrive(S, go_updown(right,sizeOfSample).location),
                 S[(location1_x+1)%(sizeOfSample*2//3)][location1_y],
                 retrive(S, go_updown(left,sizeOfSample).location),
                 left
                 ]
    count = 0
...
def adjacent_plat(S, node1, node2, sizeOfSample):
    ...
    if x1 == x2:
        if y1 == 0 and y2 == sizeOfSample - 1:
            bottom_node_list.append(retrive(S,go_right(node2,sizeOfSample).location))
            bottom_node_list.append(retrive(S,go_left(node2,sizeOfSample).location))
        elif y2 == 0 and y1 == sizeOfSample - 1:
            bottom_node_list.append(retrive(S,go_right(node1,sizeOfSample).location))
            bottom_node_list.append(retrive(S,go_left(node1,sizeOfSample).location))
        elif y1 < y2:
            bottom_node_list.append(retrive(S,go_right(node1,sizeOfSample).location))
            bottom_node_list.append(retrive(S,go_left(node1,sizeOfSample).location))
        else:
