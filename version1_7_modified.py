...
def run(S,sizeOfSample,shuffle_timestep,loop_timestep,mid_term,temperature,J,k_B,mcstp):
    ...
    while True:
        ...
        for i in range(mcstp):
            ...
            if (E1<=1): 
                E = E_after
                #refresh_accept += 1  I don't think it is necessary to take this into account 
            else:
                E = E_previous
                S = updateLoop(S,path)
                #refresh_reject += 1  I don't think it is necessary to take this into account 
            if(E==-sizeOfSample*sizeOfSample/3):
                print('T='+str(temperature)+',times of reaching ground state:'+str(count2))
                gsop = orderparameter(S,sizeOfSample)
                print("ground state order parameter: " + str(gsop))
                flag = False
                break
            if(count2>=30):
                count2 = 0
...
if __name__ == "__main__":
    ...
    configuration = (shuffle_timestep,loop_timestep,mid_term,temperature,E_per_star,k_B,sizeOfSample,mcstp)
